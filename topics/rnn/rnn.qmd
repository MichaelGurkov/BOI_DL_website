...
# CPI Example: Recurrent Neural Network for Inflation Forecasting
...
```{python }
#| code-fold: true
#| code-summary: "Create CPI sequences using per-series function"


# === Parameters ===
WINDOW = 24   # past months window


# -----------------------------------------------------------
# Function 1: build sequences for a single CPI series
# -----------------------------------------------------------
def create_sequences_single_series(values, window=12):
    """
    values: 1D numpy array of the target for a single item_id.
    Output:
        X_i: (n_i, window, 1)
        y_i: (n_i,)
    """
    ...
    if len(values) <= window:
        return None, None  # skip short series

    for i in range(window, len(values)):
        seq = values[i-window:i]
        X_i.append(seq.reshape(window, 1))
        y_i.append(values[i])
...
# -----------------------------------------------------------
# Function 2: apply to all item_id
# -----------------------------------------------------------
def create_panel_sequences(df, window=12):
    """
    df: columns ['item_id', 'timestamp', 'target']
    Returns pooled X, y across all series.
    """
    X_all = []
    y_all = []

    for item, df_item in df.groupby("item_id"):
        vals = df_item["target"].values
        X_i, y_i = create_sequences_single_series(vals, window)

        if X_i is not None:
            X_all.append(X_i)
            y_all.append(y_i)

    # concatenate pooled sequences
...
# === Create sequences ===
X_cpi, y_cpi = create_panel_sequences(cpi_raw, window=WINDOW)

# Show resulting shapes
X_cpi.shape, y_cpi.shape


```


```{python split_and_scale_cpi_data}
#| code-fold: true
#| code-summary: "Train/test split and scaling (CPI)"
...
Xc_train, Xc_test, yc_train, yc_test = train_test_split(
    X_cpi, y_cpi, test_size=0.2, shuffle=False
)

# === Scale inputs ===
scaler_Xc = StandardScaler().fit(Xc_train.reshape(-1, 1))

Xc_train_scaled = scaler_Xc.transform(Xc_train.reshape(-1, 1)).reshape(Xc_train.shape)
Xc_test_scaled  = scaler_Xc.transform(Xc_test.reshape(-1, 1)).reshape(Xc_test.shape)

# === Scale target ===
scaler_yc = StandardScaler().fit(yc_train.reshape(-1, 1))

yc_train_scaled = scaler_yc.transform(yc_train.reshape(-1, 1)).flatten()
yc_test_scaled  = scaler_yc.transform(yc_test.reshape(-1, 1)).flatten()

# Preview shapes
Xc_train_scaled.shape, yc_train_scaled.shape, Xc_test_scaled.shape, yc_test_scaled.shape
...
```{python }
#| code-fold: true
#| code-summary: "Build and train CPI RNN/LSTM/GRU models"
...
INPUT_SHAPE_CPI = (WINDOW, 1)


# -----------------------------------------------------------
# Function to build a model with a chosen recurrent cell
# -----------------------------------------------------------
def build_cpi_model(cell):
    model = tf.keras.Sequential([
        cell(12, activation='tanh', input_shape=INPUT_SHAPE_CPI),
        tf.keras.layers.Dense(1)
    ])
...
# === Build models ===
model_cpi_rnn  = build_cpi_model(tf.keras.layers.SimpleRNN)
model_cpi_lstm = build_cpi_model(tf.keras.layers.LSTM)
model_cpi_gru  = build_cpi_model(tf.keras.layers.GRU)


# -----------------------------------------------------------
# Training helper (same as in Dow Jones block)
# -----------------------------------------------------------
def train_cpi_model(model, X, y):
    return model.fit(
        X, y,
        epochs=EPOCHS,
        batch_size=BATCH_SIZE,
...
# === Train all CPI models ===
hist_cpi_rnn  = train_cpi_model(model_cpi_rnn,  Xc_train_scaled, yc_train_scaled)
hist_cpi_lstm = train_cpi_model(model_cpi_lstm, Xc_train_scaled, yc_train_scaled)
hist_cpi_gru  = train_cpi_model(model_cpi_gru,  Xc_train_scaled, yc_train_scaled)
